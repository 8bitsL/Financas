...
exports.addGastosVariaveis = (req, res) => {
	try {
		...
		novosGastosVariaveis.forEach(item => {
			...
			const itemExiste = "SELECT id FROM Gastos_Variaveis WHERE id = ? AND valido = 'true'"

			db.all(itemExiste, id, (err, rows) => {

				if (rows.length > 0) {
					const atualizaItemExistente = "UPDATE Gastos_Variaveis SET label = ?, valor = ?, pago = ? WHERE id = ?"
